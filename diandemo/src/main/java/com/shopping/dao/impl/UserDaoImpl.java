...
public class UserDaoImpl implements IUserDao {
	...
	public UserInfo selectUserInfoByUsername(String username) {
		...
		try {
			  ...
			  if(rs.next()) {
				  ...
				  userInfo=new UserInfo(id,username1, password, email, phone,role, question, answer);
			  }
			  return userInfo ;
	...
	public boolean deleteUser(int id) {
		return SqlVisiter.executeUpdate(
	...
	public boolean updateUser(UserTable user) {
		return SqlVisiter.executeUpdate(
			"update user_table set username=?,password=?,phone=?,email=?,role=?,question=?,answer=?,update_time=NOW() where id=?", 
			user.getUsername(),user.getPassword(),user.getPassword(),user.getEmail(),
			user.getRole(),user.getQuestion(),user.getAnswer(),user.getId()
		)==1;
	}
}
